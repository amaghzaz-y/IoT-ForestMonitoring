...
export async function getLastData(): Promise<EndDevice[]> {
	try {
		const metrics = await prisma.metric.findMany({
			take: 50,
			orderBy: { date: "desc" },
		...
		for (const metric of metrics) {
			const dev: EndDevice = {
				...
				lux: metric.lux,
				sound: metric.sound,
				movement: metric.movement ? 1 : 0,
				flame: metric.flame ? 1 : 0,
				eui: metric.eui,
			};
			devs.push(dev);
		}
		...
		console.error("Error fetching last 200 day summaries:", error);
		return [];
	} finally {
		await prisma.$disconnect();
	}
